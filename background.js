...
const alarms = {
  ...
  getAll(c) {
    ...
    chrome.alarms.getAll((alarms) => c([...locals, ...alarms]));
  },
};
alarms.cache = {};
...
const silent = (id) => {
  ...
  alarms.getAll((as) =>
    as
      .filter((a) => a.name.startsWith('audio-' + id))
      .forEach((a) => alarms.clear(a.name))
  );
};
...
chrome.runtime.onMessage.addListener((request, sender, respose) => {
  if (request.method === 'set-alarm') {
    ...
    const sets = request.jobs
      .filter((j) => j.method === 'set-alarm')
      .map((j) => j.name);
    const clears = request.jobs
      .filter((j) => j.method === 'clear-alarm')
      .filter((j) => sets.indexOf(j.name) === -1);
    for (const job of clears) {
      alarms.clear(job.name);
    }
    request.jobs
      .filter((j) => j.method === 'set-alarm')
      .forEach((j) => alarms.create(j.name, j.info));
